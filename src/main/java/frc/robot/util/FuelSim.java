...
public class FuelSim {
  ...
  public FuelSim(String tableKey) {
    ...
    // Keep existing constructor arg behavior, but use it as AK log path
  ...
  // replace StructArrayPublisher with a log key
  protected final String fuelLogKey;

  /** Adds array of fuel positions to AdvantageKit log/RLOG stream */
  public void logFuels() {
    Logger.recordOutput(
        fuelLogKey, fuels.stream().map((fuel) -> fuel.pos).toArray(Translation3d[]::new));
  }
