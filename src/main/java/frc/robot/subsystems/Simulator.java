  ...
  private List<RegressionTest> regressionTestCases() {
    return switch (regressTest) {
      ...
      case SUBSYSTEM_TEST_BOTH -> List.of(
          new RegressionTest("Auto test", AutoName.L_HALF_SWEEP_SHOOT, TeleopScenario.AUTO_ROTATE , Alliance.Blue));
    ...
    return switch (teleopScenario) {
      ...
      case TURRET -> List.of(
          ...
          new SimEvent(
              ...
              "Spin",
              EventType.MOVE_JOYSTICK_TURN,
              new Pose2d(0.0, 0.5, Rotation2d.kZero)),
          new SimEvent(
              t += 15.0,
              "Spin",
              EventType.MOVE_JOYSTICK_TURN,
              new Pose2d(0.0, -0.5, Rotation2d.kZero)),
          new SimEvent(t += 15.0, "Final Movement", EventType.END_OF_SCENARIO));
